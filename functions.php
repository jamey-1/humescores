	...
	function humescores_setup() {
		...
		register_nav_menus(
			array(
				...
				'menu-2' => esc_html__( 'Social Media Menu', 'humescores' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'humescores_custom_background_args',
				array(
...
// 	 */
// 	$libre_franklin = _x( 'on', 'Libre Franklin font: on or off', 'humescores' );
...
function humescores_scripts() {
	...
	// wp_enqueue_style( 'humescores-fonts', 'https://fonts.googleapis.com/css2?family=PT+Serif:ital,wght@0,400;0,700;1,400;1,700&family=Source+Sans+Pro:ital,wght@0,400;0,600;0,900;1,400&display=swap' );
	wp_enqueue_style( 'humescores-fonts', humescores_fonts_url() );

	wp_enqueue_style( 'humescores-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'humescores-style', 'rtl', 'replace' );

	wp_enqueue_script( 'humescores-navigation', get_template_directory_uri() . '/js/navigation.js', array('jquery'), _S_VERSION, true );
	...
	wp_enqueue_script( 'humescores-functions', get_template_directory_uri() . '/js/functions.js', array('jquery'), _S_VERSION, true );
